...
**pluginlib** 包提供了一系列工具使得 ROS 程序可以动态的加载或者卸载插件。这里的插件一般指动态库，插件的目的往往是类似的，但是达到目的的方法却不尽相同，装载不同的插件，就能使程序获得不同的表现。为了方便， *pluginlib* 允许程序根据插件名以及插件注册的路径，而动态的加载。这一过程是不需要重新编译的，所以配合 ROS 的动态参数的机制，可以很容易的实现只更改参数文件中装载的插件名，便可使用不同的算法，快速地对比各个算法的优缺点，以及找到最合适某个场景的算法。

###　插件的编写方法

*pluginlib* 利用多态的特性，可互相更换的插件均实现统一接口。用户在使用插件时，调用统一的接口函数，更换时由于接口相同，所以并不需要更改程序。

所以，使用 *pluginlib* 的编写插件的方法可总结为以下三步：
...
在 *calculate_base.h* 中声明插件基类 *Calculate*，定义了接口函数，分别为 *initialize* 初始化函数，以及 *result* 运算函数。
...
在　calculate_plugin.h　中，实现了两个插件类，分别是 *Add* 和 *Multiply*，它们均实现了基类 *Calculate* 的接口函数。这里，为了方便我将两个插件的实现写到了一个文件中，这并不影响它们成为两个插件，看完接下来的教程会发现，**插件是基于类的**，而不是基于动态库。
